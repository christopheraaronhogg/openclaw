// JSON-RPC plugin entries bridge synchronous OpenClaw registration to child-process runtimes.
...
export type JsonRpcPluginProcessOptions = {
  command: string;
...
export type JsonRpcPluginToolRegistration = {
  type: "tool";
  name: string;
  description: string;
  parameters?: JsonRpcPluginJsonObject;
...
export type JsonRpcPluginHookRegistration = {
  type: "hook";
  hook: PluginHookName;
  description?: string;
...
export type JsonRpcPluginHttpRouteRegistration = {
  type: "httpRoute";
  path: string;
...
export type JsonRpcPluginGatewayMethodRegistration = {
  type: "gatewayMethod";
  method: string;
  scope?: OperatorScope;
  rpcMethod?: string;
...
export type JsonRpcPluginRegistration =
  | JsonRpcPluginToolRegistration
  | JsonRpcPluginHookRegistration
  | JsonRpcPluginHttpRouteRegistration
  | JsonRpcPluginGatewayMethodRegistration;

export type JsonRpcPluginEntryOptions = {
  id: string;
  name: string;
  description: string;
...
export function defineJsonRpcPluginEntry(
  options: JsonRpcPluginEntryOptions,
): OpenClawPluginDefinition {
...
function registerJsonRpcTool(
  ...
  api.registerTool(tool, registration.options);
}
...
function registerJsonRpcHook(
  ...
  if (!isPluginHookName(registration.hook)) {
    throw new Error(`unknown JSON-RPC plugin hook: ${String(registration.hook)}`);
  }
  api.on(
    registration.hook,
...
class JsonRpcPluginClient {
  ...
  private ensureStarted(): void {
    ...
    const child = spawn(spawnInvocation.command, spawnInvocation.argv, {
      cwd: this.options.cwd ?? this.api.rootDir,
