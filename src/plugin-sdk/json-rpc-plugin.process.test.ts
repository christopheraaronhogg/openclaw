...
import { defineJsonRpcPluginEntry } from "./json-rpc-plugin.js";
import { createTestPluginApi } from "./plugin-test-api.js";

describe("defineJsonRpcPluginEntry process management", () => {
  afterEach(() => {
    vi.useRealTimers();
    vi.clearAllMocks();
  });
  ...
  it("resolves child commands through the Windows spawn resolver", async () => {
    ...
    const entry = defineJsonRpcPluginEntry({
...
function createProcessTestApi(overrides: Partial<OpenClawPluginApi> = {}): OpenClawPluginApi {
  return createTestPluginApi({
    id: "json-rpc-process-test",
    name: "JSON RPC Process Test",
    source: "src/plugin-sdk/json-rpc-plugin.process.test.ts",
    rootDir: process.cwd(),
    ...overrides,
  });
}
